...
(function () {
    ...
    async function runPageLogic(pageName, params) {
        if (pageName === "productos" && typeof window.renderProductos === "function") {
            await window.renderProductos();
            return;
        }

        if (pageName === "producto" && typeof window.renderProducto === "function") {
    ...
    async function loadPage(pageName, params = {}, options = {}) {
        ...
        try {
            ...
            app.innerHTML = "<div class='container'><p>Error cargando la pagina.</p></div>";
            console.error(error);
        }
    }
